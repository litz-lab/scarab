...
struct ptag_struct {
  ...
  bool operator==(const ptag_struct& other) const {
    return (pc == other.pc) && (ch0 == other.ch0) && (ch1 == other.ch1);
  }

  bool operator!=(const ptag_struct& other) const { return !(*this == other); }
};

using GTAG_t = std::variant<uint, ptag_struct>;
...
// Template class for PredictorStates
...
struct PredictorStates {
  ...
  std::unique_ptr<ptag_struct[]> GTAG;

  // Constructor
  PredictorStates(bool snapshot = false) {
    int size = snapshot ? NOSKIPCNT : (NHIST + 1);
    GI = std::make_unique<int[]>(size);
    GTAG = std::make_unique<ptag_struct[]>(size);
  }
#define GTAG(GTAG) (GTAG.pc ^ GTAG.ch0 ^ (GTAG.ch1 << 1))
...
#define INDUSEALT(state) (((((state.HitBank - 1) / 8) << 1) + state.AltConf) % (SIZEUSEALT - 1))
 ...
 public:
  ...
  int F_infinite(long long A, int size, int bank);
  int gindex(unsigned int PC, int bank, long long hist, cbp64_folded_history* ch_i);
  int gindex_infinite(unsigned int PC, int bank, long long hist, cbp64_folded_history* ch_i);
  uint gtag(unsigned int PC, int bank, cbp64_folded_history* ch0, cbp64_folded_history* ch1);
  ptag_struct ptag(unsigned int PC, int bank, cbp64_folded_history* ch0, cbp64_folded_history* ch1);
