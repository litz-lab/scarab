...
void BTBMissBPTakenConfStat::log_phase_cycles(Op* op) {
  if (!CONFIDENCE_ENABLE || !CONF_LOG_PHASE_CYCLES)
    return;
  Off_Path_Reason op_reason = (Off_Path_Reason)op->bp_pred_main.off_path_reason;
  switch (op_reason) {
...
void BTBMissBPTakenConf::per_cf_op_update(Op* op, Conf_Off_Path_Reason& new_reason) {
  if (CONF_BTB_MISS_BP_TAKEN_CONF && !CONF_PERFECT_BTB_MISS_CONF && op->btb_pred.btb_miss &&
      (op->bp_pred_main.pred_orig == TAKEN) && (op->bp_confidence >= CONF_BTB_MISS_BP_TAKEN_THRESHOLD)) {
