...
void Decoupled_FE::recover() {
  ...
  Addr addr2 = 0;
  if (DUAL_DFE_ENABLE && DUAL_DFE_POLICY == DFE2_Policy::CONTINUE_ON_RECOVERY) {
  ...
  if (DUAL_DFE_ENABLE) {
    if (DUAL_DFE_POLICY == DFE2_Policy::CONTINUE_ON_RECOVERY) {
      frontend_redirect2(proc_id, addr2);
      dfe2->set_off_path();
    } else if (DUAL_DFE_POLICY == DFE2_Policy::CONTINUE_ON_PREDICTION) {
...
void Decoupled_FE::update() {
  ...
  while(1) {
    ...
    if(op->table_info->cf_type) {
      ...
      Op alt_op;
      if (DUAL_DFE_ENABLE &&
          (DUAL_DFE_POLICY == DFE2_Policy::CONTINUE_ON_PREDICTION) &&
          !dfe2->is_off_path())
        alt_op = *op;
      ...
      if(op->oracle_info.recover_at_decode || op->oracle_info.recover_at_exec) {
        ...
        if (DUAL_DFE_ENABLE &&
          ...
          Addr alt_pred_addr = bp_predict_op(g_bp_data2, &alt_op, 0, op->inst_info->addr);
...
void Decoupled_FE2::update() {
  ...
  while(1) {
    ...
    if (BP_MECH != MTAGE_BP && !bp_is_predictable(g_bp_data2, proc_id)) {
      DEBUG(proc_id, "Break due to limited branch predictor\n");
      if (off_path)
    ...
    if(op->table_info->cf_type) {
      ASSERT(proc_id, op->eom);
      pred_addr = bp_predict_op(g_bp_data2, op, cf_num++, op->inst_info->addr);
      DEBUG(proc_id,
            "Predict CF fetch_addr:%llx true_npc:%llx pred_npc:%lx mispred:%i misfetch:%i btb miss:%i taken:%i recover_at_decode:%i recover_at_exec:%i off_path:%i bar_fetch:%i\n",
            op->inst_info->addr, op->oracle_info.npc, pred_addr,
            op->oracle_info.mispred, op->oracle_info.misfetch,
