...
void TAGE64K::UpdatePredictor (UINT64 PC, OpType opType, bool resolveDir,
  ...
  int IndBIAS = (((((PC ^ (PC >> 2)) << 1) ^ 
                 (state.LowConf & (state.LongestMatchPred != state.alttaken))) << 1) + 
  ...
  cbp64_gentry& hit_entry = (state.HitBank > 0) ? 
    gtable[state.HitBank][state.GI[hit_idx]] : dummy_entry;
  
  cbp64_gentry& alt_entry = (state.AltBank > 0) ? 
    gtable[state.AltBank][state.GI[alt_idx]] : dummy_entry;
