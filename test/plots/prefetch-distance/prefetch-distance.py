...
filename="prefetch-distance.txt"
if len(sys.argv) > 1:
  filename = sys.argv[1]
...
for j in range(1,len(data[0])):
    item=data[0][j]
    ax.plot(data[1][data[0][0]], data[1][item], color=colors[j%len(colors)])

ax.set_ylim(ymin=0,ymax=60)
ax.set_xlim(xmin=0,xmax=48)
ax.set_ylabel(r'\% of ideal BTB performance')
ax.set_xlabel(r'Prefetch distance (in cycles)')
# ax.legend(ncol=3, prop={'size': 16}, columnspacing=0.25)
ax.grid(linestyle='--',zorder=0)
plt.axhline(0, color='gray')
plt.tight_layout()
fig.savefig("prefetch-distance.pdf",bbox_inches='tight',pad_inches=0.1)