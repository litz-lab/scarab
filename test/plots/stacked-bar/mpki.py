...
def add_subplot(filename, ax, ytitle, show_legend=False, logy=False):  # , loc):
    ...
    print(dimw)
    print("data0: ", data[0])
    num_cols_to_size={5: 'xx-small', 3: '21', 1: '21'}
    # if 'twig' in data[1]:
    ...
    ideal_x = x[verilator_index] + (data[0].index('MPKI') - 1) * dimw - dimw/5
    error=7.5 - ideal_x
    print(ideal_x)
    print("Error 2: ", error, "; num dimw: ", error / dimw)
    ax.text(ideal_x, cutoff*1.25, "{0:.0f}".format(data[1]['MPKI'][7]), rotation='vertical', size=num_cols_to_size[dim])
