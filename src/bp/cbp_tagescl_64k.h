...
public:
  ...
  void UpdatePredictor (UINT64 PC, OpType opType, bool resolveDir, bool predDir, UINT64 branchTarget, const PredictorStates& state);
  void NonSpecUpdateAtCond (UINT64 PC, OpType opType, bool resolveDir, bool predDir, UINT64 branchTarget, Counter key);
  ...
  void ComparePredictor(const PredictorStates& state);
  void CompareCheckpoint(const Checkpoint& state);
  ...
  // component tracker for prediction
