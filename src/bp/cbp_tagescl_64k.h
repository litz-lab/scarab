...
#define NOSKIPCNT 21  // the number of history lengths which are used in CBP64K
...
struct Checkpoint {
  ...
  cbp64_folded_history bch_i[NHIST + 1];     // folded global history using cyclic shift register to generate i'th TAGE index
  cbp64_folded_history bch_t[2][NHIST + 1];  // folded global history using cyclic shift register to generate i'th TAGE tag
  ...
  int8_t       GGEHL[GNB][(1 << LOGGNB)] = {{0}};  // GEHL component which exploits 'GHIST'
  int8_t       PGEHL[PNB][(1 << LOGPNB)] = {{0}};  // GEHL component which exploits 'phist'
  int8_t       WG[(1 << 3)];                       // GGEHL's weight table
  int8_t       WP[(1 << 3)];                       // PGEHL's weight table
  cbp64_lentry ldata[1 << (LOGL)];                 // entire loop table.
};
...
struct PredictorEntry {
  ...
  PredictorEntry(const Counter& c, PredictorStates&& s)
      : counter(c), state(std::move(s)) {}
};

typedef boost::multi_index_container<PredictorEntry,
...
struct CheckpointEntry {
  ...
  CheckpointEntry(const Counter& c, const Checkpoint& cp)
      : counter(c), state(cp) {}
};

// Define the multi_index_container
typedef boost::multi_index_container<CheckpointEntry,
    // Ordered by Counter as the key (unique index)
    boost::multi_index::indexed_by<boost::multi_index::ordered_unique<
...
#define IMNB 2
  ...
  int8_t* GGEHL[GNB];  // N: GEHL for global history
  // variation on global branch history
  int     Pm[PNB]                    = {25, 16, 9};
  int8_t  PGEHLA[PNB][(1 << LOGPNB)] = {{0}};
  int8_t* PGEHL[PNB];  // N: GEHL for path history
  // first local history
...
#define INDUPDS ((PC ^ (PC >> 2)) & ((1 << (LOGSIZEUPS)) - 1))
  int8_t WG[(1 << LOGSIZEUPS)];   // S: GEHL weights for global history
  ...
  int8_t WP[(1 << LOGSIZEUPS)];   // N: GEHL weights for path history
...
#define INDUSEALT (((((Pstate.HitBank - 1) / 8) << 1) + Pstate.AltConf) % (SIZEUSEALT - 1))
 ...
 public:
  ...
  void            baseupdate(bool Taken, UINT64 PC, cbp64_bentry* btable);
  Counter         MYRANDOM(Counter op_num);
  void            Tagepred(UINT64 PC);
  bool            GetPrediction(UINT64 PC, int* bp_confidence, Op* op);
  void            SpecUpdate(UINT64 PC, OpType opType, bool dir, bool pred, UINT64 target);
  void            SpecUpdateAtCond(UINT64 PC, bool dir, bool pred);
  void            HistoryUpdate(UINT64 PC, OpType opType, bool taken, UINT64 target, int hitbank);
  void            UpdatePredictor(UINT64 PC, OpType opType, bool resolveDir, bool predDir, UINT64 branchTarget, Counter op_num, const PredictorStates& state);
  void            NonSpecUpdateAtCond(UINT64 PC, OpType opType, bool resolveDir, bool predDir, UINT64 branchTarget, Counter key, Counter op_num);
  ...
  void            TrackOtherInst(UINT64 PC, OpType opType, bool taken, UINT64 branchTarget, Counter key);
  int             lindex(UINT64 PC);
  int             lib(UINT64 PC);
  int             ltag(UINT64 PC);
  bool            getloop(UINT64 PC);
  int             GetBrtypeFromOptype(OpType opType);
  void            GlobalStateUpdate(UINT64 PC, UINT64 target, int brtype, bool dir);
  void            SpecLoopUpdate(UINT64 PC, bool Taken);
  void            LoopUpdate(UINT64 PC, bool Taken, bool ALLOC, Counter op_num, int lhit);
  PredictorStates Pstate;  // P: predictor states
  ...
  cbp64_lentry*        ltable;                   // S: loop predictor table
  long long            GHIST;                    // S: global history
  uint8_t              ghist[HISTBUFFERLENGTH];  // S: global history buffer
  int                  ptghist;                  // S: path history index
  long long            phist;                    // S: path history
  cbp64_folded_history ch_i[NHIST + 1];          // S: utility for computing TAGE indices
  cbp64_folded_history ch_t[2][NHIST + 1];       // S: utility for computing TAGE tags
  ...
  Counter   Seed;       // for the pseudo-random number generator
  ...
  void TakeCheckpoint(UINT64 PC, Counter key, OpType optype, Cf_Type cftype);
  void SavePredictorStates(UINT64 PC, Counter key, uns proc_id);
  void RetireCheckpoint(Counter key);
  void RestorePredictorstates(Counter key, UINT64 PC);
  void RestoreCheckpoint(UINT64 PC, Counter key, uns proc_id);
  void VerifyCheckpoint(UINT64 PC, Counter key, uns proc_id);
  virtual void RepairStateAndUpdate(UINT64 PC, bool new_dir, UINT64 branchTarget, Recovery_Info* recovery_info);
  ...
  bool                off_path;
  Counter             condpattern;  // N: condition pattern
  int8_t              noskip_index[NHIST + 1];
};

class TAGE64K2 : public TAGE64K {
public:
  TAGE64K2(void);
  void RepairStateAndUpdate(UINT64 PC, bool new_dir, UINT64 branchTarget, Recovery_Info* recovery_info);
};
