...
void FDIP::update() {
  ...
  for (op = decoupled_fe_ftq_iter_get(dfe, ftq_idx, &end_of_block); op != NULL;
    ...
    if (line_addr != last_line_addr) {
      ...
      emit_new_prefetch = determine_usefulness(line_addr, op, op->bp_pred_main.pred_global_hist);
      ...
      if (!emit_new_prefetch && !line && !mem_req)  // TODO: whether to insert the candidate for the subsidiary
        insert_pref_candidate_to_seniority_ftq(line_addr, op->bp_pred_main.pred_global_hist);
      if (!bp_id && (FDIP_UTILITY_HASH_ENABLE || FDIP_UC_SIZE || FDIP_BLOOM_FILTER))
...
void FDIP::log_stats_path_conf_per_pref_candidate() {
  ...
  if (!is_conf_off_path()) {
    if (is_off_path()) {
      ...
      STAT_EVENT(proc_id, FDIP_OFF_CONF_ON_NOT_IDENTIFIED_PREF_CANDIDATES + cur_op->bp_pred_main.off_path_reason);
    } else
...
void FDIP::log_stats_path_conf_emitted() {
  ...
  if (!is_conf_off_path()) {
    ...
    if (is_off_path()) {
      ...
      STAT_EVENT(proc_id, FDIP_OFF_CONF_ON_NOT_IDENTIFIED_EMITTED + cur_op->bp_pred_main.off_path_reason);
    } else {
