...
// void LVCP::spec_update(Addr pc, Op* op, const bool resolve_dir,
//                                                const bool pred_dir, const Addr next_pc) {
...
void LVCP::notify_instr_decode(Addr pc, Op* op) {
    ...
    if (map_it == set.end()) {
        ...
        // stats
        uint64_t current_bitmap_usage =
            std::accumulate(load_marking_table.begin(), load_marking_table.end(), uint64_t{0},
