...
class Boolean_Sequence {
 public:
  Boolean_Sequence(uns64 size, double t_nt_ratio)
      : size(size), next_element_index(0), boolean_array(size), dist(t_nt_ratio), normal_dist(0.5) {
    for (auto &item : boolean_array)
      item = static_cast<uns16>(dist(rand_engine));
  }
  ...
  bool _get_next_element(Sequence_Pick_Strategy strategy) {
    switch (strategy) {
      ...
      default:
        return boolean_array[next_element_index++];
    }
  }
 ...
 private:
  ...
  std::bernoulli_distribution normal_dist;
};
