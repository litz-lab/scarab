...
void ft_info_to_orders_insert() {
  ...
  for (Addr pc : FT_buffer[wrptr_lb].get_all_pc()) {
...
Op* FT_buffer_read_Op(uns proc_id) {
  if (!FT_buffer[rdptr_lb].can_fetch_op()) {
    ...
    if (deferred_addr.size() > 0 &&
        (FT_buffer[rdptr_lb].get_ft_info().static_info.start == *deferred_addr.begin() ||
         FT_buffer[rdptr_lb + off_path_ft].get_ft_info().static_info.start == deferred_addr[0])) {
...
void lookahead_buffer_ft_removed_from_ftq(uns proc_id, Addr addr) {
  if (FT_buffer[rdptr_lb_in_ftq + deferred_addr.size()].get_ft_info().static_info.start == addr) {
