...
/* Reads from frontend until a complete FT is ready, inserts it into the buffer;
   also used during initialization to prefill the buffer */
void FT_buffer_insert_FT(uns8 proc_id);

/* Returns a uop from the buffer to the DFE */
Op* FT_buffer_read_Op(uns proc_id);

/* Returns a uop to the frontend; handles both on-path and off-path fetching;
   advances FT read pointer when FT is fully consumed */
...
/* Called when DFE goes off-path to perform redirect recovery */
void lookahead_buffer_redirect();

/* Called to recover lookahead buffer state after misprediction */
