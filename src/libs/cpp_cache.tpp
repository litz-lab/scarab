  ...
  if(!inserting_addr_orders){
    ...
    for (uint64_t i = rd_ptr_pos; i < scan_pos; i++) {
      ...
      if(curr_set == target_set){
        ...
        if(!not_in_candi){
          auto it = uop_count_map.find(ft_info_in.start);
          if (it != uop_count_map.end()) {
            auto &vec = it->second;
            bool found = false;
            for (auto &entry : vec) {
              if (entry.first == ft_info_in.n_uops) {
                ...
                if (!entry.second) {
                  ...
                  if (available_space < required_length)
                    return 1;
                  entry.second = true;  // mark as counted
                }
                break;
              }
            }
            ...
            uop_count_map[ft_info_in.start] = {{ft_info_in.n_uops, false}};
          }
        }
      }

    }
    return (available_space > 0 &&  available_space >= required_length) ? 0 : 1;
  }
}
