...
class CBP_To_Scarab_Intf {
 ...
 public:
  CBP_CLASS* get_predictor(uns proc_id) { return &cbp_predictors.at(proc_id); }
...
// Specialization for TAGE64K2
template <>
uns8 CBP_To_Scarab_Intf<TAGE64K2>::pred(Op* op) {
  uns proc_id = op->proc_id;
  if (op->off_path) {
    if (SPEC_LEVEL < BP_PRED_ONOFF_SPEC_UPDATE_S_ONOFF_N_ON)
      return op->oracle_info.dir;
...
template <>
void CBP_To_Scarab_Intf<TAGE64K2>::spec_update(Op* op) {
  uns    proc_id        = op->proc_id;
  OpType optype         = scarab_to_cbp_optype(op);
  Flag   is_conditional = is_conditional_branch(op);
...
template <>
void CBP_To_Scarab_Intf<TAGE64K2>::update(Op* op) { /* CBP Interface does not support update at exec */
  if (op->off_path)
    return;
  if (SPEC_LEVEL < BP_PRED_ONOFF_SPEC_UPDATE_S_ONOFF_UPDATE_N_ON)
    return;
...
template <>
void CBP_To_Scarab_Intf<TAGE64K2>::retire(Op* op) {
  if (SPEC_LEVEL == BP_PRED_ON)
    return;
  uns proc_id = op->proc_id;
  cbp_predictors.at(proc_id).RetireCheckpoint(op->recovery_info.branch_id);
...
template <>
void CBP_To_Scarab_Intf<TAGE64K2>::recover(Recovery_Info* recovery_info) {
  if (SPEC_LEVEL == BP_PRED_ON)
    return;
  uns proc_id = recovery_info->proc_id;
  cbp_predictors.at(proc_id).RepairStateAndUpdate(
...
template <>
void CBP_To_Scarab_Intf<TAGE64K2>::timestamp(Op* op) {
  uns proc_id                 = op->proc_id;
  op->recovery_info.branch_id = cbp_predictors.at(proc_id).KeyGeneration();
}
...
void sync_predictors(uns proc_id) {
  TAGE64K2* tage2 = cbp_predictor_TAGE64K2.get_predictor(proc_id);
  TAGE64K* tage = cbp_predictor_TAGE64K.get_predictor(proc_id);
  *tage2 = *tage;
}
