...
void TAGE64K::Tagepred(UINT64 PC) {
  ...
  if (HitBank > 0) {
    if (AltBank > 0) {
      ...
      tage_component_alt = (AltBank >= BORN) ? TAGE_LONG : TAGE_SHORT;  // tag hit at the longer history length: TAGE_LONG
    } else
      alttaken = getbim();
...
bool TAGE64K::GetPrediction(UINT64 PC, int* bp_confidence, Op* op) {
  ...
  tage_component = ((WITHLOOP >= 0) && (LVALID)) ? TAGE_LOOP : tage_component_tage;  // update STATUS to loop predictor or keep component level
#endif
  ...
  if (pred_inter != SCPRED) {
    ...
    if (HighConf) {
      ...
      // if |LSUM| is 'slightly' smaller than threshold(THRES / 4 <= |LSUM| < THRES / 2), select pred_inter when SecondH >= 0
  ...
  if (!op->off_path) {
    ...
    STAT_EVENT(op->proc_id, TAGESCL_COMP_BTB_HIT_TAGE_BASE_CORRECT + (pred_taken != op->oracle_info.dir) + tage_component * 2 + op->oracle_info.btb_miss * 12);
  }
...
void TAGE64K::UpdatePredictor(UINT64 PC, OpType opType, bool resolveDir,
  ...
  if ((SCPRED != resolveDir) || ((abs(LSUM) < THRES))) {
    ...
    {
      int XSUM =
          LSUM - ((WB[INDUPDS] >= 0) * ((2 * Bias[INDBIAS] + 1) + (2 * BiasSK[INDBIASSK] + 1) + (2 * BiasBank[INDBIASBANK] + 1)));
      if ((XSUM +
